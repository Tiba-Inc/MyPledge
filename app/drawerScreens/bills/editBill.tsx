...
const EditBill = () => {
  ...
  const bill = BillsData.find((b) => b.id.toString() === id);
  ...
  useEffect(() => {
    if (bill) {
      reset({
        ...
        date: bill.date,
        notifications: bill.notifications || 'None',
      });
      setSelectedFrequency(bill.frequency);
      setSelectedBillCategory(bill.billCategory);
      setSelectedDate(bill.date);
      setSelectedNotification(bill.notifications || 'None');
    }
  }, [bill, reset]);
  ...
  const onSubmit = (data: {
    ...
    // Update the bill in your data store
    const updatedBills = BillsData.map((b) =>
      b.id.toString() === id ? { ...b, ...data, amount: Number(data.amount) } : b
    );
    console.log('Updated bill:', updatedBills);
    router.back(); // Go back to the bill details screen
  };
  ...
  return (
    <View className="mx-4">
      <Stack.Screen options={{ headerTitle: 'Edit Bill' }} />
